...
```{r}
holiday_rast<-rast("C:/Users/Bryce/OneDrive - University Of Oregon/Data_Sci/ds-environ/labs/data/soil-burn-severity/2020_holidayfarm_sbs/HolidayFarm_SBS_final.tif")
beachie_rast<-rast("C:/Users/Bryce/OneDrive - University Of Oregon/Data_Sci/ds-environ/labs/data/soil-burn-severity/2020_beachiecreek_sbs/BeachieCreek_SBS_final.tif")
terwilliger_rast<-rast("C:/Users/Bryce/OneDrive - University Of Oregon/Data_Sci/ds-environ/labs/data/soil-burn-severity/2018_terwilliger_sbs/SoilSeverity.tif")
...
-ANSWER: terwilliger looks really squished for whatever reason. High burn serveiry is close to the middle of the fire foot prints for all, There is a lot more unburned in terwilligers too.  
...
```{r}
crs(holiday_rast, proj =TRUE)#NAD83 / UTM zone 10N / meters
crs(beachie_rast, proj = TRUE) #NAD83# meters
crs(terwilliger_rast, proj = TRUE) # NAD83, meters
```

- ANSWER crs: all are NAD83 but holiday is a UTM, beachie is  Albers conic, and terwilliger isAlbers
- ANSWER units: Holiday: meters Beachie: meters Terwilliger: meteres
- ANSWER the same? : The same? No
...
```{r}
terwilliger_rast<- project(terwilliger_rast, holiday_rast, res = holiday_rast)
...
```{r}
twilly<-as.character(terwilliger_df$SoilBurnSe)

twilly[terwilliger_df$SoilBurnSe == "High"] <- "4"
twilly[terwilliger_df$SoilBurnSe == "Moderate"] <- "3"
twilly[terwilliger_df$SoilBurnSe == "Low"] <- "2"
twilly[terwilliger_df$SoilBurnSe == "Unburned"] <- "1"

twilly <- as.factor(twilly)
terwilliger_df$SoilBurnSe <- twilly
...
ggplot() +
 # geom_tile(data = beachie_df, aes(x = x, y = y, fill = Layer_1)) +
  geom_tile(data = holiday_df, aes(x = x, y = y, fill = Layer_1)) +
  geom_tile(data = terwilliger_df, aes(x = x, y = y, fill = SoilBurnSe)) +
  scale_fill_brewer(palette = "Spectral", direction = -1) 
...
```{r oreco}
or_region<-st_read("C:/Users/Bryce/OneDrive - University Of Oregon/Data_Sci/ds-environ/labs/data/OR-ecoregions/Ecoregions_OregonConservationStrategy.shp")
```
...
or_region<-st_transform(or_region, st_crs(holiday_rast))
```
...
ggplot() + 
  ...
  geom_tile(data = terwilliger_df , 
                 aes(x = x, y = y, 
                  fill = SoilBurnSe)) + 
  scale_fill_brewer(palette = "Spectral", direction = -1) 
 

      #scale_fill_viridis_d(option = "C") #had to change the color scale so it could fit all the color symbols needed to show all the data, was loosing burn servryts 1 and 2 before.
```
...
holiday_df$Layer_1 <- as.factor(holiday_df$Layer_1) # I do not think this

ggplot(holiday_df, aes(x = Layer_1,
                       y =(..count..)/sum(..count..),
...
#ggplot(beachie_df, aes(x = Layer_1,
                       #y =(..count..)/sum(..count..),
                       #fill = as.factor(Layer_1))) +
 # geom_bar() +
#  scale_fill_brewer(palette = "Spectral", direction = -1)

ggplot(terwilliger_df, aes(x = SoilBurnSe,
                       y =(..count..)/sum(..count..),
...
ANSWER: Holiday farm first had the highest proportion of moderate severity compared to terwilliger, who highest proportion was low severity. Also holiday had higher high severity than terwilliger, so overall the holiday farm fire was a more server fire both in intensity and size. This is in line with what Haldofsky 2020 is prediciting, that due to climate change, wildfires are becoming larger and more serivty in the PNW. 
