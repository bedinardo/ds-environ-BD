...
```{r}
probEat <- function(pred_mass, prey_mass){
  log_ratio <- log(pred_mass/prey_mass)
  p <- alpha + beta *log_ratio + gamma * (log_ratio)^2
  prob<-inv_logit(p)
  round(prob, 2)
}
...
sq_matrix<-matrix(NA, nrow = nrow(sp_occ), ncol = nrow(sp_occ))#empty square matrix

colnames(sq_matrix)<- rownames(sp_occ)
rownames(sq_matrix)<- rownames(sp_occ)

for(i in 1:nrow(sp_occ)){
  for(j in 1:nrow(sp_occ)){
    a_mass <- sp_mass[i,"f"]
    b_mass<- sp_mass[j, "f"]
   sq_matrix[i,j]<- probEat(a_mass, b_mass)
  }
}
head(sq_matrix)
...
```{r}
carnivores
inter_act <- sweep(sq_matrix, 2, carnivores, `*`)
print(rowSums(inter_act))
print(colSums(inter_act))
...
t2_interactions<-sweep(inter_act, 2, present_v2, `*`) #by cols
t2_interactions<-bipartite::empty(t2_interactions)
sum((rowSums(t2_interactions) > 0))

test<-inter_act * present_v2
sum(rowSums(test) > 0)

#key sweep function but works
t2 <- sweep(inter_act, MARGIN=1, sp_occ[,2], `*`)
sum(rowSums(t2) > 0)
#Answer
t2 <- inter_act * present_v2
t2 <- sweep(t2, MARGIN=2, sp_occ[,2], `*`)
sum(rowSums(t2) > 0)
...
```{r}
num_points<-ncol(sp_occ)
network_list<-vector("list", length = num_points)

for (t in 1:num_points) {
  present_time <- sp_occ[, t]
  
  # Recompute t2 from scratch each time
  t2 <- inter_act * present_v2
  t2 <- sweep(t2, MARGIN = 2, present_time, `*`)
  time_interaction <- sweep(t2, MARGIN = 1, present_time, `*`)
  
  network_list[[t]] <- time_interaction
}
...
#t2_interactions<-empty(inter_act)
#t2_interactions<-sweep(t2_interactions, 1, present_v2, `*`) #by rows
#t2_interactions<-sweep(t2_interactions, 2, present_v2, `*`) #by cols
#active_rows <- rowSums(t2_interactions > 0) > 0
#sum(active_rows)
...
```{r}
graphing_list <- lapply(network_list, function(mat) {
  graph_from_adjacency_matrix(mat, mode = "directed", weighted = TRUE, diag = FALSE)
})
 #it says that I need square matrix, but they can't be and won't be square because preds are columns and prey is rows, and they are different numbers
...
groups <- carnivores[V(graphing_list[[1]])$name]

###  convert to a network 3d object

foodweb_net3d <- igraph_to_networkD3(graphing_list[[1]], group=groups,
                                       what = "both")
...
```{r}
groups_5 <- carnivores[V(graphing_list[[5]])$name]
foodweb_net3d_5 <- igraph_to_networkD3(graphing_list[[5]], group=groups,
                                       what = "both")
forceNetwork(Links = foodweb_net3d_5$links,
             Nodes = foodweb_net3d_5$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group",
            opacity = 0.8, zoom=TRUE, opacityNoHover = 0.9)
...
```{r}
groups_10 <- carnivores[V(graphing_list[[10]])$name]
foodweb_net3d_10 <- igraph_to_networkD3(graphing_list[[10]], group=groups,
                                       what = "both")
forceNetwork(Links = foodweb_net3d_10$links,
             Nodes = foodweb_net3d_10$nodes,
            Source = "source", Target = "target",
            Value = "value",  NodeID = "name",
             Group = "group",
            opacity = 0.8, zoom=TRUE, opacityNoHover = 0.9)
